...
def download_youtube_playlist(playlist_url, base_folder):
    ydl_opts = {
        ...
        'postprocessors': [{
            ...
            'preferredcodec': 'm4a',  # Preferred codec for ALAC
...
def convert_to_alac(m4a_file):
    audio = AudioSegment.from_file(m4a_file, format='m4a')
    alac_file = m4a_file.replace('.m4a', '.alac')
    audio.export(alac_file, format='alac')
    os.remove(m4a_file)
    return alac_file

def process_audio_files(folder_path):
    m4a_files = glob.glob(os.path.join(folder_path, '*.m4a'))
    print(f"Found {len(m4a_files)} M4A files in {folder_path}")

    for m4a_file in m4a_files:
        print(f"Processing file: {m4a_file}")
        alac_file = convert_to_alac(m4a_file)
        
        track_name = os.path.basename(alac_file).replace('.alac', '')
